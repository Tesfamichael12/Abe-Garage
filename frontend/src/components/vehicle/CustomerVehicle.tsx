  ...
  return (
    <div className="px-4 py-5 md:flex gap-10 ">
      ...
      <div className="flex-grow">
        <p className="text-2xl font-bold text-customBlue mb-2">Vehicles</p>
          ...
          <table className="min-w-full bg-white border border-gray-200 mt-3">
            {vehicles && vehicles.length > 0 ? (
              ...
              <div className="shadow-2xl">
                <p className="text-customeRed text-center p-4 font-semibold">
                  No vehicles found
                </p>
              </div>
            )}
        ...
        <div
          className={`relative shadow-full max-w-[700px] p-4 md:p-10 mt-10 ${
            showForm ? "block" : "hidden"
          }`}
        >
          <button
            className="bg-customeRed text-white absolute top-0 right-0  p-2  w-8 h-8 flex items-center justify-center translate-x-1/2 -translate-y-1/2"
            onClick={() => removeForm()}
          >
            X
          </button>
          ...
          <form className="mt-10" onSubmit={handleSubmit(onSubmit)}>
            <input
              ...
              className="block w-full sm:w-4/5 p-3 mb-4 border border-gray-300  rounded"
              placeholder="Vehicle year"
            />
            {errors.vehicle_year && (
              <p className="text-customeRed">{errors.vehicle_year.message}</p>
            )}
            <input
              {...register("vehicle_make", {
                required: "Vehcile make is required",
                minLength: {
                  value: 3,
                  message: "Vehicle make must be at least 3 characters",
                },
              })}
              className="block w-full sm:w-4/5 p-3 mb-4 border border-gray-300  rounded "
              placeholder="Vehcile make"
              type="text"
            />
            {errors.vehicle_make && (
              <p className="text-customeRed">{errors.vehicle_make.message}</p>
            )}
            <input
            ...
            {errors.vehicle_model && (
              <p className="text-customeRed">{errors.vehicle_model.message}</p>
            )}
            <input
            ...
            />
            {errors.vehicle_type && (
              <p className="text-customeRed">{errors.vehicle_type.message}</p>
            )}
            <input
            ...
            {errors.vehicle_color && (
              <p className="text-customeRed">{errors.vehicle_color.message}</p>
            )}
            <input
            ...
            {errors.vehicle_mileage && (
              <p className="text-customeRed">
                {errors.vehicle_mileage.message}
              </p>
            )}
            <input
              {...register("vehicle_tag", {
                required: "Vehicle tag is required",
              })}
              className="block w-full sm:w-4/5 p-3 mb-4 border border-gray-300  rounded"
            ...
            />
            {errors.vehicle_tag && (
              <p className="text-customeRed">{errors.vehicle_tag.message}</p>
            )}
            <input
              {...register("vehicle_serial", {
                required: "Vehicle serial is required",
              })}
              className="block w-full sm:w-4/5 p-3 mb-4 border border-gray-300  rounded"
            ...
            {errors.vehicle_serial && (
              <p className="text-customeRed">{errors.vehicle_serial.message}</p>
            )}
            <button
              disabled={isSubmitting}
              className="bg-customeRed px-5 py-3 text-white font-semibold "
              type="submit"
            >
              {isSubmitting ? (
        ...
        <button
          className={`bg-customeRed  text-white py-4 px-7  mt-4 rounded-md ${
