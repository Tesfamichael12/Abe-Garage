...
async function createVehicle(req, res, next) {
  try {
    ...
    const vehicle = await vehicleService.getVehicleBySerial(
...
async function getVehicleByCustomerId(req, res, next) {
  try {
    const vehicle = await vehicleService.getVehicleByCustomerId(
    ...
    console.error("Error in getVehicleByCustomerId controller:", error);
    return res.status(500).json({
      status: "Fail",
      message: "something went wrong",
    });
